    ...
    function setValue(int $color, int $shade, int $bright, int $wbright, string $status) {
        ...
        if ($status == "on") {
            if ($this->type == 'RGB' || $this->type == 'RGBW') {
                ...
                $method = Objects::getMethodByAlias($this->idObject, "ch1,2,3_color");
                Action::runAction($method->id, null, null, $color);

                $method = Objects::getMethodByAlias($this->idObject, "ch1,2,3_shade");
                Action::runAction($method->id, null, null, $shade);

                $method = Objects::getMethodByAlias($this->idObject, "ch1,2,3_bright");
                Action::runAction($method->id, null, null, $bright);

                $method = Objects::getMethodByAlias($this->idObject, "ch1,2,3_enable");
                if ($bright <= 1) {
                    Action::runAction($method->id, null, null, 0);
                } else {
                    Action::runAction($method->id, null, null, 1);
                }
            ...
            elseif ($this->type == 'W') {
                ...
                $method = Objects::getMethodByAlias($this->idObject, "ch4_single_bright");
                Action::runAction($method->id, null, null, $bright);
