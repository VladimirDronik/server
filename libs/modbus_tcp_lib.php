...
function modbusTcpCrc($modbusMsg){
...
function modbusTcpSendCmd ($packet, $host, $port) 
    {
...
// TODO 
    // Добавить проверку возможных неисправностей. Выставить флаг в БД.
function modbusTcpCheckResponse ($response)
{
    if (!$response)
    {
        echo "[ERR] Something went wrong! Response is not recieved\n";
    }
    else
    {
        echo "[OK] Everything is OK! Response is recieved.\n";
        return $response;
    }
}

function modbusTcpTransactionId ()
{
    $transactionId = str_pad((dechex((string)(rand(0,65535)))), 4, "0", STR_PAD_LEFT); // Transaction ID
